...
from ..representations.atom37_converter import (
...
class Atom37:
    ...
    def __post_init__(self) -> None:
        ...
        if self.coords.shape != expected_coords_shape:
            raise ValueError(
                f"坐标张量形状无效: {self.coords.shape}，期望 {expected_coords_shape}"
            )
        ...
        if self.atom_mask.shape != expected_atom_mask_shape:
            raise ValueError(
                f"原子掩码张量形状无效: {self.atom_mask.shape}，期望 {expected_atom_mask_shape}"
            )
        ...
        if self.atom_names.shape != (37,):
            raise ValueError(
                f"原子名称张量形状无效: {self.atom_names.shape}，期望 (37,)"
            )

        # 验证可选属性
        if (
            self.b_factors is not None
            and self.b_factors.shape != expected_atom_mask_shape
        ):
            raise ValueError(
                f"B因子张量形状无效: {self.b_factors.shape}，期望 {expected_atom_mask_shape}"
            )

        if (
            self.occupancies is not None
            and self.occupancies.shape != expected_atom_mask_shape
        ):
            raise ValueError(
                f"占用率张量形状无效: {self.occupancies.shape}，期望 {expected_atom_mask_shape}"
            )
    ...
    def to_device(self, device: torch.device) -> "Atom37":
        ...
        occupancies = (
            self.occupancies.to(device) if self.occupancies is not None else None
        )
    ...
    def get_residue_atoms(self, residue_idx: int) -> Dict[str, torch.Tensor]:
        ...
        # 构建字典，只包含真实存在的原子
        result = {}
        for atom_idx, (atom_name, coord, is_real) in enumerate(
            zip(atom_names_list, residue_coords, residue_mask)
        ):
            if is_real:
    ...
    def compute_center_of_mass(self) -> torch.Tensor:
        ...
        # 计算每个残基的真实原子数量
        num_valid_atoms = self.atom_mask.sum(
            dim=-1, keepdim=True
        )  # (..., num_residues, 1)
        ...
        # 计算质心
        center_of_mass = valid_coords.sum(dim=-2) / num_valid_atoms.squeeze(
            -1
        )  # (..., num_residues, 3)

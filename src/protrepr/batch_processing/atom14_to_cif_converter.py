"""
Atom14 到 CIF/PDB 批量转换核心实现

提供高效的批量 Atom14 到 CIF/PDB 转换功能，支持并行处理、
多种输出格式和完整的错误处理。
"""
...
class BatchAtom14ToCIFConverter:
    """
    批量 Atom14 到 CIF/PDB 转换器。
    ...
    def find_atom14_files(
        ...
        files = sorted(files)
        logger.info(f"在 {input_path} 中找到 {len(files)} 个 Atom14 文件")
        return files
    ...
    def convert_batch(
        ...
        """
        # 查找所有 Atom14 文件
        atom14_files = self.find_atom14_files(input_path, recursive)
        
        if not atom14_files:
            logger.warning("没有找到 Atom14 文件")
            return {'total': 0, 'success': 0, 'failed': 0, 'failed_files': [], 'results': []}
        
        # 准备转换任务
        tasks = []
        for file_path in atom14_files:
            # 确定输出路径

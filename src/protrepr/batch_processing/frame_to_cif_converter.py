"""
Frame 到 CIF/PDB 批量转换核心实现

提供高效的批量 Frame 到 CIF/PDB 转换功能，支持并行处理、
多种输出格式和完整的错误处理。
"""
...
class BatchFrameToCIFConverter:
    """
    批量 Frame 到 CIF/PDB 转换器。
    ...
    def __init__(
        ...
        logger.info(f"Frame 到 {self.output_format.upper()} 批量转换器初始化完成:")
    ...
    def find_frame_files(
        ...
        files = sorted(files)
        logger.info(f"在 {input_path} 中找到 {len(files)} 个 Frame 文件")
        return files
    ...
    def convert_batch(
        ...
        """
        # 查找所有 Frame 文件
        frame_files = self.find_frame_files(input_path, recursive)
        
        if not frame_files:
            logger.warning("没有找到 Frame 文件")
        ...
        if self.n_workers == 1:
            ...
            with ProcessPoolExecutor(max_workers=self.n_workers) as executor:
                # 提交所有任务
                future_to_task = {
                    executor.submit(convert_frame_to_cif_worker, str(input_file), str(output_file), 
...
def convert_frame_to_cif_worker(
    input_file: str,
    output_file: str,
    output_format: str
) -> Dict[str, Any]:

...
这个脚本提供命令行接口，用于批量将结构文件（PDB/CIF/ENT/MMCIF）转换为 ProtRepr 的 Atom37 格式。
...
使用方法:
    python batch_struct_to_atom37.py input_dir output_dir [options]

示例:
    # 基本用法 (保存为 Atom37 实例)
    python batch_struct_to_atom37.py /path/to/structure_files /path/to/output
    
    # 保存为字典格式
    python batch_struct_to_atom37.py /path/to/structure_files /path/to/output --save-as-dict
    
    # 使用并行处理
    python batch_struct_to_atom37.py /path/to/structure_files /path/to/output --workers 8
    
    # 不保持目录结构
    python batch_struct_to_atom37.py /path/to/structure_files /path/to/output --no-preserve-structure
    
    # 指定设备
    python batch_struct_to_atom37.py /path/to/structure_files /path/to/output --device cuda
"""

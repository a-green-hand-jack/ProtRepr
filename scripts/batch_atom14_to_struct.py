...
这个脚本提供命令行接口，用于批量将 ProtRepr Atom14 格式文件转换为结构文件（CIF 或 PDB）。
核心实现位于 protrepr.batch_processing 模块中。

输入格式: .pt (PyTorch 格式的 Atom14 数据)
支持的输出格式: .cif, .pdb

使用方法:
    python batch_atom14_to_struct.py input_dir output_dir [options]
...
示例:
    ...
    # 使用并行处理
    python batch_atom14_to_struct.py /path/to/atom14_files /path/to/output --workers 8
    
    # 不保持目录结构
    python batch_atom14_to_struct.py /path/to/atom14_files /path/to/output --no-preserve-structure
"""
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="批量将 ProtRepr Atom14 文件转换为结构文件（CIF/PDB）格式",

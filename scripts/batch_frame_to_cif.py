#!/usr/bin/env python3
"""
批量 Frame 到 CIF/PDB 转换脚本

这个脚本提供命令行接口，用于批量将 ProtRepr Frame 格式文件转换为 CIF 或 PDB 文件。
核心实现位于 protrepr.batch_processing 模块中。

使用方法:
    python batch_frame_to_cif.py input_dir output_dir [options]

示例:
    # 基本用法 (转换为 CIF 格式)
    python batch_frame_to_cif.py /path/to/frame_files /path/to/output
    
    # 转换为 PDB 格式
    python batch_frame_to_cif.py /path/to/frame_files /path/to/output --format pdb
    
    # 使用并行处理
    python batch_frame_to_cif.py /path/to/frame_files /path/to/output --workers 8
    
    # 不保持目录结构
    python batch_frame_to_cif.py /path/to/frame_files /path/to/output --no-preserve-structure
"""
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="批量将 ProtRepr Frame 文件转换为 CIF/PDB 格式",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
示例:
  %(prog)s /data/frame_files /data/cif_output
  %(prog)s /data/frame_files /data/pdb_output --format pdb --workers 8

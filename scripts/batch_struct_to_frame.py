...
这个脚本提供命令行接口，用于批量将结构文件（PDB/CIF/ENT/MMCIF）转换为 ProtRepr 的 Frame 格式。
...
使用方法:
    python batch_struct_to_frame.py input_dir output_dir [options]

示例:
    # 基本用法 (保存为 Frame 实例)
    python batch_struct_to_frame.py /path/to/structure_files /path/to/output
    
    # 保存为字典格式
    python batch_struct_to_frame.py /path/to/structure_files /path/to/output --save-as-dict
    
    # 使用并行处理
    python batch_struct_to_frame.py /path/to/structure_files /path/to/output --workers 8
    
    # 不保持目录结构
    python batch_struct_to_frame.py /path/to/structure_files /path/to/output --no-preserve-structure
    
    # 指定设备
    python batch_struct_to_frame.py /path/to/structure_files /path/to/output --device cuda
"""
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="批量将结构文件（PDB/CIF/ENT/MMCIF）转换为 ProtRepr Frame 格式",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
示例:
  %(prog)s /data/structure_files /data/frame_output
  %(prog)s /data/structure_files /data/frame_output --workers 8 --save-as-dict
